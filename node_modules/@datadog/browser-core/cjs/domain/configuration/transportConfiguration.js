...
function computeTransportConfiguration(initConfiguration, buildEnv, isIntakeV2Enabled) {
    var endpointBuilder = endpointBuilder_1.createEndpointBuilder(initConfiguration, buildEnv, isIntakeV2Enabled);
    var intakeUrls = endpointBuilder_1.ENDPOINTS_TYPES.map(function (endpointType) { return endpointBuilder.buildIntakeUrl(endpointType); });
    var configuration = {
    ...
    if (buildEnv.buildMode === init_1.BuildMode.STAGING && initConfiguration.replica !== undefined) {
        var replicaConfiguration = tslib_1.__assign(tslib_1.__assign({}, initConfiguration), { site: endpointBuilder_1.INTAKE_SITE_US, applicationId: initConfiguration.replica.applicationId, clientToken: initConfiguration.replica.clientToken, useAlternateIntakeDomains: true, intakeApiVersion: isIntakeV2Enabled ? 2 : 1 });
        var replicaEndpointBuilder_1 = endpointBuilder_1.createEndpointBuilder(replicaConfiguration, buildEnv, isIntakeV2Enabled);
        ...
        var replicaIntakeUrls = endpointBuilder_1.ENDPOINTS_TYPES.map(function (endpointType) { return replicaEndpointBuilder_1.buildIntakeUrl(endpointType); });
