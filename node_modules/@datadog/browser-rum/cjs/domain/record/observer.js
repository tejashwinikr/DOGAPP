...
function initMutationObserver(mutationController, cb, initialPrivacyLevel) {
    return mutationObserver_1.startMutationObserver(mutationController, cb, initialPrivacyLevel).stop;
}
...
function initMouseInteractionObserver(cb, initialPrivacyLevel) {
    var handler = function (event) {
        var target = event.target;
        if (privacy_1.getNodePrivacyLevel(target, initialPrivacyLevel) === constants_1.NodePrivacyLevel.HIDDEN || !serializationUtils_1.hasSerializedNode(target)) {
            return;
        }
        var _a = utils_1.isTouchEvent(event) ? event.changedTouches[0] : event, clientX = _a.clientX, clientY = _a.clientY;
        cb({
...
function initScrollObserver(cb, initialPrivacyLevel) {
    var updatePosition = browser_core_1.throttle(browser_core_1.monitor(function (event) {
        var target = event.target;
        if (!target ||
            privacy_1.getNodePrivacyLevel(target, initialPrivacyLevel) === constants_1.NodePrivacyLevel.HIDDEN ||
...
function initInputObserver(cb, initialPrivacyLevel) {
    function eventHandler(event) {
        var target = event.target;
        var nodePrivacyLevel = privacy_1.getNodePrivacyLevel(target, initialPrivacyLevel);
        if (!target ||
            !target.tagName ||
